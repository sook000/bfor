...
import { checkId, getSido, getGugun, registerUser } from "@/api/user.js";
...
const idCheckShow = ref("");
const idValidClass = ref("");
const idShowClass = ref("");
const isUseId = ref(false);
const idCheck = () => {
  const len = user.value.userId.length;
  if (len < 5 || len > 12) {
    isUseId.value = false;
    idCheckShow.value = "아이디는 5자 이상 12자 이하입니다.";
    idValidClass.value = "is-invalid";
    idShowClass.value = "invalid-feedback";
  } else {
    checkId(
      user.value.userId,
      (res) => {
        ...
        if (res.data === 0) {
          isUseId.value = true;
          idCheckShow.value = "";
          idValidClass.value = "is-valid";
          idShowClass.value = "valid-feedback";
        } else {
          isUseId.value = false;
          idCheckShow.value = "중복된 아이디입니다.";
          idValidClass.value = "is-invalid";
          idShowClass.value = "invalid-feedback";
        }
      },
...
const fileUpload = (e) => {
  user.value.profileImage = e.target.files[0];
  console.log(user.value.profileImage);
  imageUploaded.value = URL.createObjectURL(user.value.profileImage);
...
const userRegister = () => {
  ...
  //   if (user.value.userTel) {
  //     alert("전화번호 입력!");
  //     return;
  //   }
  //   if (user.value.userEmailId) {
  //     alert("이메일 아이디 입력!");
  //     return;
  //   }
  //   if (user.value.userEmailDomain) {
  //     alert("이메일 주소 입력!");
  //     return;
  //   }
  if (confirm("회원 가입을 하시겠습니까?")) {
    ...
    // formData.append("userPassword", user.value.userPassword);
    // formData.append("userName", user.value.userName);
    // formData.append("userTel", user.value.userTel);
    // formData.append("userBirth", user.value.userBirth);
    // formData.append("userEmailId", user.value.userEmailId);
    // formData.append("userEmailDomain", user.value.userEmailDomain);
    // formData.append("regdate", user.value.regdate);
    // formData.append("sidoCode", user.value.sidoCode);
    // formData.append("gugunCode", user.value.gugunCode);
    // formData.append("gugunCode", user.value.gugunCode);
    formData.append(
      "userDto",
      new Blob([JSON.stringify(user.value)], {
        type: "application/json",
      })
    );
    formData.append("profileImage", user.value.profileImage);
    axios
      .create({
      ...
      .then((response) => {
        let msg = "회원 등록 시 문제 발생했습니다";
        if (response.status == 201) msg = "회원 가입이 완료되었습니다";
        alert(msg);
...
<template>
  <div class="d-flex justify-content-center mt-5 mb-5">
    <div class="card sm-5 me-2 p-4" style="width: 55%">
      ...
      <form id="form-join" @submit.prevent="userRegister">
        <div class="row mb-5">
          <div class="ms-auto me-auto px-3 d-flex align-items-center justify-content-center">
            <img
              src="@/assets/register/identification.png"
              alt="name"
              style="width: 30px; height: 30px; margin-right: 10px"
            />
            <input
              ...
              :class="nameValidClass"
              @keyup="nameCheck"
            />
          </div>
          <div
            style="display: block"
            id="register-id-check-show"
            class="mt-1 ms-5 w-100 text-start"
            :class="nameShowClass"
          >
            {{ nameCheckShow }}
          </div>
        ...
        <div class="row mb-5">
          <div class="test ms-auto me-auto px-3 d-flex align-items-center justify-content-center">
            <img
              src="@/assets/register/doublelock.png"
              alt="doublepassword"
              style="width: 30px; height: 30px; margin-right: 10px"
            />
            <input
              ...
              required
              v-model="userDblPassword"
              :class="dblpwdValidClass"
              @keyup="dblpwdCheck"
            />
          </div>
          <div
            style="display: block"
            id="register-password-check-show"
            class="mt-1 ms-5 w-100 text-start"
        ...
        <div class="row mb-5">
          <div class="test ms-auto me-auto px-3 d-flex align-items-center justify-content-center">
            <img
              src="@/assets/register/smartphone-call.png"
              alt="phone"
              style="width: 30px; height: 30px; margin-right: 10px"
            />
            <input
        ...
        <div class="row mb-5">
          <div class="test ms-auto me-auto px-3 d-flex align-items-center justify-content-center">
            <img
              src="@/assets/register/calendar.png"
              alt="birth"
          ...
          <div class="test ms-auto me-auto px-3 d-flex align-items-center justify-content-center">
            ...
            <div class="input-group">
              <input
                ...
                name="register-email-id"
                placeholder="*이메일 아이디"
                required
                v-model="user.userEmailId"
                :class="emailIdValidClass"
                @keyup="emailIdCheck"
              />
              ...
              <select
                ...
                required
                v-model="user.userEmailDomain"
                :class="emailDomainValidClass"
                @change="emailDomainCheck"
              >
          ...
          <div class="test ms-auto me-auto px-3 d-flex align-items-center justify-content-center">
            ...
            <div class="input-group">
              <select
                ...
                id="register-region-big"
                name="register-region-big"
                aria-label="Floating label select example"
                v-model="user.sidoCode"
                @change="getRegionSmall"
              >
              ...
              <select
                ...
                id="register-region-small"
                name="register-region-small"
                aria-label="Floating label select example"
                v-model="user.gugunCode"
              >
                <option selected value="">구군선택</option>
                <option
                  v-for="region in regionSmall"
                  :key="region.gugunCode"
        ...
        <div class="row mb-5">
          <div class="test ms-auto me-auto px-3 d-flex align-items-center justify-content-center">
            <img
              src="@/assets/register/face-id.png"
              alt="picture"
              style="width: 30px; height: 30px; margin-right: 10px"
            />
        ...
        <div class="row mb-5 justify-content-center">
          <div class="col-auto">
            <img :src="imageUploaded" alt="올린 이미지" style="width: 100px; height: auto" />
          </div>
        </div>
        ...
        <div class="register-modal-btn d-flex justify-content-end">
          <button
            type="submit"
            class="btn btn-outline-info btn-sm register-close m-1"
            style="display: block"
            value="회원가입"
          >
            회원가입
